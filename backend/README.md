...
The API also runs `alembic upgrade head` before database health checks, administrator seeding, or
any application query. This makes direct Render/Uvicorn deployments safe on an empty Neon
PostgreSQL database. PostgreSQL advisory locking serializes concurrent instance migrations, and
Alembic's version table makes repeated startup upgrades idempotent. Production configuration cannot
disable `MEDIAI_DATABASE_MIGRATE_ON_STARTUP`.
